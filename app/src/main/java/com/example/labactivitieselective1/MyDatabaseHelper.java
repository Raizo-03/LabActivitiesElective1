...
public class MyDatabaseHelper extends SQLiteOpenHelper {
    ...
    private static final int DATABASE_VERSION = 2; // Incremented to force database recreation
    ...
    void updateBook(String row_id, String title, String author, String pages) {
        ...
        Toast.makeText(context, result == -1 ? "Failed update" : "Update successful", Toast.LENGTH_SHORT).show();
    }

    void deleteBook(String row_id) {
        SQLiteDatabase db = this.getWritableDatabase();
        long result = db.delete(TABLE_NAME, "_id=?", new String[]{row_id});
        Toast.makeText(context, result == -1 ? "Failed to delete" : "Deletion successful", Toast.LENGTH_SHORT).show();
    }
